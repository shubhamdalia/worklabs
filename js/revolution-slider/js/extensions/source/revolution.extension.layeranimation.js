	...
	*************************************/	
	animateTheCaptions : function(obj)  {		
		...
		var opt = obj.opt,
			...
			base_offsetx = opt.sliderType==="carousel" ? 0 : opt.width/2 - (opt.gridwidth[opt.curWintroRange]*opt.bw)/2,
			base_offsety=0,
		...
		if (opt.debugMode) {
			...
			nextli.append('<div class="helpgrid" style="width:'+(opt.gridwidth[opt.curWintroRange]*opt.bw)+'px;height:'+(opt.gridheight[opt.curWintroRange]*opt.bw)+'px;"></div>');
			var hg = nextli.find('.helpgrid');
			hg.append('<div class="hginfo">Zoom:'+(Math.round(opt.bw*100))+'% &nbsp;&nbsp;&nbsp; Device Level:'+opt.curWintroRange+'&nbsp;&nbsp;&nbsp; Grid Preset:'+opt.gridwidth[opt.curWintroRange]+'x'+opt.gridheight[opt.curWintroRange]+'</div>')
	...
	********************************************/	
	prepareSingleCaption : function(obj) {
		...
		if (opt.sliderLayout=="fullscreen") 
			obj.offsety = _._gh/2 - (opt.gridheight[opt.curWintroRange]*opt.bh)/2;

		if (opt.autoHeight=="on" || (opt.minHeight!=undefined && opt.minHeight>0))
			  obj.offsety = opt.conh/2 - (opt.gridheight[opt.curWintroRange]*opt.bh)/2;;
		...
		var handlecaption=0,
			layervisible =  _.visibility === undefined ? "oon" : makeArray(_.visibility,opt)[opt.forcedWintroRange] || makeArray(_.visibility,opt) || "ooon";
		...
		if (_nc.find('img').length>0) {
			...
			var ww = im.data('ww'),
				hh = im.data('hh'),
				fuw = _._ba =="slide" ? opt.ulw : opt.gridwidth[opt.curWintroRange],
				fuh = _._ba =="slide" ? opt.ulh : opt.gridheight[opt.curWintroRange];
			
			ww =  makeArray(im.data('ww'),opt)[opt.curWintroRange] || makeArray(im.data('ww'),opt) || "auto",
			hh =  makeArray(im.data('hh'),opt)[opt.curWintroRange] || makeArray(im.data('hh'),opt) || "auto";
		...
		if (_._ba==="slide") {
			...
			if (_._isstatic && opt.carousel!==undefined && opt.carousel.horizontal_align!==undefined && opt.sliderType==="carousel") {			
				switch (opt.carousel.horizontal_align) {
					case "center":
						obj.offsetx = 0 + (opt.ulw - (opt.gridwidth[opt.curWintroRange]*opt.bw))/2;

					break;
					case "left":
					break;
					case "right":
						obj.offsetx = (opt.ulw - (opt.gridwidth[opt.curWintroRange]*opt.bw));
					break;
				}
		...
		if (_nc.hasClass("tp-videolayer") || _nc.hasClass("tp-audiolayer") || _nc.find('iframe').length>0 || _nc.find(tag).length>0) {
			...
			var ww =  makeArray(_nc.data('videowidth'),opt)[opt.curWintroRange] || makeArray(_nc.data('videowidth'),opt) || "auto",
				hh =  makeArray(_nc.data('videoheight'),opt)[opt.curWintroRange] || makeArray(_nc.data('videoheight'),opt) || "auto";
			...
			if (ww==="auto" || (!jQuery.isNumeric(ww) && ww.indexOf("%")>0)) {
				ww = ww==="auto" ? "auto" : _._ba==="grid" ? opt.gridwidth[opt.curWintroRange]*opt.bw : _._gw;
			} else {
				ww = (parseFloat(ww)*opt.bw)+"px";
			}

			if (hh==="auto" || (!jQuery.isNumeric(hh) && hh.indexOf("%")>0)) {
				hh = hh==="auto" ? "auto" : _._ba==="grid" ? opt.gridheight[opt.curWintroRange]*opt.bw : _._gh;				
			} else {
				hh = (parseFloat(hh)*opt.bh)+"px";
			}
		...
		_.arrobj = new Object();
		_.arrobj.voa = makeArray(_.voffset,opt)[opt.curWintroRange] || makeArray(_.voffset,opt)[0];
		_.arrobj.hoa = makeArray(_.hoffset,opt)[opt.curWintroRange] || makeArray(_.hoffset,opt)[0];
		_.arrobj.elx = makeArray(_.x,opt)[opt.curWintroRange] || makeArray(_.x,opt)[0];
		_.arrobj.ely = makeArray(_.y,opt)[opt.curWintroRange] || makeArray(_.y,opt)[0];					
		...
		var vofs= _._respoffset !=="off" ? parseInt(voa,0)*opt.bw : parseInt(voa,0),			
			...
			crw = _._ba==="grid" ? opt.gridwidth[opt.curWintroRange]*opt.bw : _._gw,
			crh = _._ba==="grid" ? opt.gridheight[opt.curWintroRange]*opt.bw : _._gh;
		...
		if (_._nctype==="row") {
			if (_.columnbreak<=opt.curWintroRange) {
...
var makeArray = function(obj,opt,show) {
	...
	if (obj.length<opt.rle) {
		for (var i=1;i<=opt.curWintroRange;i++) {
			obj.push(tempw);
		}
	}
	return obj;
...
var setResponsiveCSSValues = function(obj,opt) {
	...
	if (obj)
		jQuery.each(obj,function(key,val){						
			var res_a = makeArray(val,opt)[opt.curWintroRange];
...
var calcCaptionResponsive = function(nc,opt,level,responsive) {
	...
	if (!nc.hasClass("tp-splitted")) {
		...
		if (level!="rekursive") {
			...
			var winw = obj.basealign =="slide" ? opt.ulw : opt.gridwidth[opt.curWintroRange],
				winh = obj.basealign =="slide" ? opt.ulh : opt.gridheight[opt.curWintroRange],
			...
			if (_._cbgc_auto && _._cbgc_auto.length>0) {		
				...
				if (jQuery.isArray(obj.marginLeft)) {						
					punchgs.TweenLite.set(_._cbgc_auto,{						
						borderTopWidth:   (obj.marginTop[opt.curWintroRange] * bh) + "px",
						borderLeftWidth:  (obj.marginLeft[opt.curWintroRange] * bw) + "px",
						borderRightWidth: (obj.marginRight[opt.curWintroRange] * bw) + "px",
						borderBottomWidth:(obj.marginBottom[opt.curWintroRange] * bh) + "px",									
...
var setColumnBgDimension = function(nc,opt) {
	...
	if (_._cbgc_man && _._cbgc_man.length>0) {						
		...
		if (!jQuery.isArray(_.cssobj.marginLeft)) {			
			...
			_l = (_.cssobj.marginLeft[opt.curWintroRange] * opt.bw);
			_t = (_.cssobj.marginTop[opt.curWintroRange] * opt.bh);
			_b = (_.cssobj.marginBottom[opt.curWintroRange] * opt.bh);
			_r = (_.cssobj.marginRight[opt.curWintroRange] * opt.bw);
		}				

...
jQuery.extend(true, _R, {
	...
	setSize : function(opt) {
		...
		var ofh = (opt.top_outer || 0) + (opt.bottom_outer || 0),
			...
			maxhei = opt.gridheight[opt.curWintroRange],
			__mh = 0,
		...
		opt.height = Math.round(opt.gridheight[opt.curWintroRange] * (opt.width/opt.gridwidth[opt.curWintroRange]));

		if (opt.height>opt.gridheight[opt.curWintroRange] && opt.autoHeight!="on") opt.height=opt.gridheight[opt.curWintroRange];

		if (opt.sliderLayout=="fullscreen" || opt.infullscreenmode) {
			opt.height = opt.bw * opt.gridheight[opt.curWintroRange];
...
//////////////////////////////////////////
var setCurWintroRange = function(opt,vis) {		
	var curlevel = 0,
		curwidth = 9999,
		lastmaxlevel = 0,
		lastmaxid = 0,
	...
	if (!vis)
		opt.curWintroRange = curid;				
	else
		opt.forcedWintroRange = curid;
...
var initSlider = function (container,opt) {
 ...
 var runSlider = function(container,opt) {
	...
	if (opt.sliderLayout=="fullwidth" && opt.autoHeight=="off") container.css({maxHeight:opt.gridheight[opt.curWintroRange]+"px"});
	...
	setCurWintroRange(opt);
	setCurWintroRange(opt,true);
...
var setScale = function(opt) {
	opt.bw = (opt.width / opt.gridwidth[opt.curWintroRange]);
	opt.bh = (opt.height / opt.gridheight[opt.curWintroRange]);
	...
	jQuery('.rev_redraw_on_blurfocus').each(function() {				
		...
		if (opt.windowfocused!=true) {
		    ...
		    punchgs.TweenLite.delayedCall(0.3,function(){        	
		        ...
		        if (opt.fallbacks.nextSlideOnWintrodowFocus=="on") opt.c.revnext();
		        opt.c.revredraw();
